...
def unprocess(image):
  ...
  # save_rgb(np.array(image.permute(1, 2, 0)), 'prev_image.jpg')

  # Approximately inverts global tone mapping.
  image = inverse_smoothstep(image)
  # save_rgb(np.array(image.permute(1, 2, 0)), 'invert_tone_image.jpg')
  
  # Inverts gamma compression.
  image = gamma_expansion(image) 
  # save_rgb(np.array(image.permute(1, 2, 0)), 'invert_gamma_image.jpg')

  # Inverts color correction.
  image = apply_ccm(image, rgb2cam) 
  # save_rgb(np.array(image.permute(1, 2, 0)), 'invert_color_correction.jpg')

  # Approximately inverts white balance and brightening.
  image = safe_invert_gains(image, rgb_gain, red_gain, blue_gain) 
  # save_rgb(np.array(image.permute(1, 2, 0)), 'invert_white_balance.jpg')

  # Clips saturated pixels.
  image = torch.clamp(image, min=0.0, max=1.0) 

  # save_rgb(np.array(image.permute(1, 2, 0)), 'clip_saturated_image.jpg') 
